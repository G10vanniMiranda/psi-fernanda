...
export default function Header() {
    ...
    return (
        <header
            className={`fixed top-0 left-0 w-full z-50 transition-shadow bg-white/70 backdrop-blur-sm ${scrolled ? "shadow-sm" : "shadow-none"
                }`}
        >
                ...
                <div className="flex items-center justify-between h-16">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col leading-tight">
                                ...
                                <span className="text-xs text-slate-500">Psicóloga Clínica</span>
                            </div>
                    ...
                    <nav className="hidden md:flex items-center gap-6">
                        {links.map(([label, href]) => (
                            <a key={href} href={href} className="text-slate-700 hover:text-slate-900 text-sm">
                                {label}
                            </a>
                        ))}
                    </nav>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        >
                            <button type="button" className="bg-emerald-600 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-emerald-700 transition cursor-pointer">
                                Agendar Consulta
                            </button>
                        </Link>
            ...
            {/* Mobile menu */}
            <div className={`md:hidden ${open ? "block" : "hidden"} border-t border-slate-100 bg-white/90`}>
                <div className="px-4 pt-2 pb-4 flex flex-col gap-2">
                    {links.map(([label, href]) => (
                        <a key={href} href={href} onClick={() => setOpen(false)} className="py-2 px-2 rounded text-slate-700 hover:bg-slate-50">
                            {label}
                        </a>
                    ))}
